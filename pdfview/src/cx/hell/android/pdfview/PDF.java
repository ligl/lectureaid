...
public class PDF {
	 ...
	 * shouldn't be used anywhere outside of pdf-related code.
	 */
	...
	synchronized private native int parseFileDescriptor(FileDescriptor fd,
			int box, String password);

	/**
	 * Construct PDF structures from bytes stored in memory.
	 */
	/*
	 * public PDF(byte[] bytes, int box) { this.parseBytes(bytes, box); }
	 */
	 ...
	 */
	synchronized public native int[] renderPage(int n, int zoom, int left,
			int top, int rotation, boolean gray, boolean skipImages,
			PDF.Size rect);
